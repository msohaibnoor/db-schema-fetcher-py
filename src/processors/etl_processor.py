...
class ETLProcessor:
    ...
    def _create_llm_context(self) -> Dict[str, Any]:
        ...
        metadata = self.processed_schema.get("metadata", {})
        ...
        for table_name, table_info in tables.items():
            table_summaries[table_name] = {
                ...
                "key_columns": [col["name"] for col in table_info.get("columns", [])[:5]],  # Top 5 columns
                "primary_key": table_info.get("primary_key", []),
                "row_count": table_info.get("row_count"),
                "business_purpose": table_info.get("business_context", {}).get("likely_purpose", "unknown")
            }
        
        return {
            "database_description": f"Database '{metadata.get('database_name', 'unknown')}' with {len(tables)} tables",
